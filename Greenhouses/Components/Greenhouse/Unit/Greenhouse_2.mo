...
model Greenhouse_2
  "Venlo-type greenhouse for tomato crop cultivated from 10Dec-22Nov (weather data from TMY)"
  ...
  Modelica.Fluid.Interfaces.FluidPort_b flangeB(redeclare package Medium =
        ...
        transformation(extent={{-212,-104},{-192,-84}}), iconTransformation(
          extent={{-176,-158},{-156,-138}})));
  Modelica.Fluid.Interfaces.FluidPort_a flangeA(redeclare package Medium =
        Modelica.Media.Water.ConstantPropertyLiquidWater) annotation (Placement(
        transformation(extent={{-40,-176},{-20,-156}}), iconTransformation(
          extent={{-176,-60},{-156,-40}})));
  Modelica.Blocks.Interfaces.RealOutput PID_Mdot_CS annotation (Placement(
        transformation(extent={{156,-110},{136,-90}}),iconTransformation(extent={{156,
            -104},{176,-84}})));
equation
  ...
  connect(PID_CO2.CS, MC_ExtAir.U_MCext) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.Bezier));
  connect(pipe_low.pipe_out, flangeB) annotation (Line(
      points={{-61,-125},{-129.5,-125},{-129.5,-94},{-202,-94}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(pipe_low.pipe_in, flangeA) annotation (Line(
      points={{-37,-125},{-34.5,-125},{-34.5,-166},{-30,-166}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(PID_Mdot_CS, PID_Mdot.CS) annotation (Line(
      points={{146,-100},{158,-100},{158,-98},{169.4,-98}},
      color={0,0,127},
      smooth=Smooth.None));
  annotation (
