...
model SmoothedInit
  ...
  extends
    Greenhouses.Flows.FluidFlow.HeatTransfer.BaseClasses.PartialHeatTransferSmoothed(
      massFlowExp=0.0, redeclare replaceable package Medium =
...
//   x_V   = 1+max(smoothingRange/divisor,10*Modelica.Constants.small);
  ...
  initialized = Greenhouses.Functions.transition_factor(
    start=t_start,
    stop=t_start + t_init,
    position=time);
  ...
  der(U_filtered) = (U_initialized-U_filtered)/filterConstant;
   ...
   if max_dUdt<=0 then
    ...
    dUdt_limited = (Greenhouses.Functions.transition_factor(
      start=-max_dUdt,
      stop=max_dUdt,
      position=(U_filtered - U_limited)/1e-5) - 0.5)*2;
